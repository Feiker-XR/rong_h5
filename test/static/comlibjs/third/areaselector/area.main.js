...
var areaSelectorClass = function () {

	this.areaInit = function () {
		var thirdPath = $$r.rootUrl + '/comlibjs/third/areaselector/';
		(function() {
		...
		function sendCitiesAjax() {
			$.ajax({
				type: "get",
				url: thirdPath + 'queryAllCities.js',
				async: true,
				dataType: "json",
		...
		function viewHotCities() {
			$.each(cities, function(i, city) {
				if (city.hotCity) {
					$(".hotCity .list ul").append("<li><a><input city_provinceId='"+city.provinceId+"' city_id='"+city.id+"' city_name='"+city.name+"' type='button' style='background:none;border:0px;cursor: pointer;' id='" + city.id + "' value='" + city.name + "'></a></li>");
					// $(".hotCity .list ul").append("<li><a><input type='button' style='background:none;border:0px;cursor: pointer;' ondbclick=hotCityAddrInput(\'" + city.provinceId + "," + city.id + "," + city.name + "\')></a></li>");
				}
			});
			// ondbclick=hotCityAddrInput(\'" + city.provinceId + "," + city.id + "," + city.name + "\') id='" + city.id + "' value='" + city.name + "'
			$(".hotCity .list ul li input").on('click', function () {
				var m = $(this)
				hotCityAddrInput(m.attr('city_provinceId')+','+m.attr('city_id') + ','+m.attr('city_name'))
			})
		}
		...
		function viewProvince(page) {
			...
			for (var i = start; i < end; i++) {
				...
				var li = $('<li><a style="background: none repeat scroll 0% 0% transparent; border: 0px none;" viewcity_i='+i+' href="javascript:;" id="' + p_id + '">' + p_name + '</a></li>');
				$(".province .list ul").append(li);
			}
			$(".province .list ul li a").on('click', function () {
				var m = $(this)
				viewCities(m.attr('viewcity_i'))
			})
		...
		function cityPage(page) {
			...
			for (var i = start; i < end; i++) {
				...
				var li = $('<li><a viewcounties_i="'+i+'" href="javascript:;" id="' + c_id + '">' + cityName + '</a></li>');
				$(".city .list ul").append(li);
			}
			$(".city .list ul li a").on('click', function () {
				var m = $(this)
				viewCounties(m.attr('viewcounties_i'))
			})
		...
		function countyPage(page) {
			...
			for (var i = start; i < end; i++) {
				...
				var li = $('<li><a addrInput_i="'+i+'" href="javascript:;" id="' + c_id + '">' + countyName + '</a></li>');
				$(".county .list ul").append(li);
			}
			$(".city .list ul li a").on('click', function () {
				var m = $(this)
				addrInput(m.attr('addrInput_i'))
			})
		...
		function sendAllProvinceAjax() {
			$.ajax({
				type: "get",
				url: thirdPath + 'queryAllProvinces.js',
				async: true,
				dataType: "json",
				success: function(data) {
		...
		function sendAllCountiesAjax()
		 {
			$.ajax({
				type: "get",
				url: thirdPath + "queryAllAreas.js",
				async: true,
				dataType: "json",
				success: function(data) {
					allAreas = data.areas;
		...
		function viewAllHotCities() {
			$.each(allCities,function(i, city) {
				if (city.hotCity) {
					...
					$(".hotCityAll .list ul").append("<li><a><input type='button' style='background:none;border:0px;cursor: pointer;' city_provinceId='"+city.provinceId+"' city_id='"+city.id+"' city_name='"+city.name+"' id='" + city.id + "' value='" + city.name + "'></a></li>");
				}
			});
			$(".hotCityAll .list ul li input").on('click', function () {
				var m = $(this)
				hotCityAddrInputAll(m.attr('city_provinceId')+','+m.attr('city_id') + ','+m.attr('city_name'))
			})
		}
		...
		function viewAllProvince(page) {
			...
			for (var i = start; i < end; i++) {
				...
				var li = $('<li><a style="background: none repeat scroll 0% 0% transparent; border: 0px none;" viewallcities_i="' + i + '" href="javascript:;" id="' + p_id + '">' + p_name + '</a></li>');
				$(".provinceAll .list ul").append(li);
			}
			$(".provinceAll .list ul li a").on('click', function () {
				var m = $(this)
				viewAllCities(m.attr('viewallcities_i'))
			})
		...
		function allCityPage(page) {
			...
			for (var i = start; i < end; i++) {
				...
				var li = $('<li><a viewallcounties_i="'+i+'" href="javascript:;" id="' + c_id + '">' + cityName + '</a></li>');
				$(".cityAll .list ul").append(li);
			}
			$(".cityAll .list ul li a").on('click', function () {
				var m = $(this)
				viewAllCounties(m.attr('viewallcounties_i'))
			})
		...
		function allCountyPage(page) {
			...
			for (var i = start; i < end; i++) {
				...
				var li = $('<li><a addrinputall_i="'+i+'" href="javascript:;" id="' + c_id + '">' + countyName + '</a></li>');
				$(".countyAll .list ul").append(li);
			}
			$(".countyAll .list ul li a").on('click', function () {
				var m = $(this)
				addrInputAll(m.attr('addrinputall_i'))
			})
...
module.exports = areaSelectorClass;
