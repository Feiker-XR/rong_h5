...
seajs.use(['Ajax','dot','tips2',"Dialog",'sortable'], 
    ...
    var setModel = function() {
        ...
        var imageslibraryUrl = $$r.routers.data.link.imageslibrary,
            ...
            keywordUrl = $$r.routers.data.link.keyword,
        ...
        var form = new $rk.VC({
            ...
            releaseRequest:function(tipBotton){
        ...
        var main = new $rk.VC({
            _init: function() {
                ...
                $rk.listener.on('keyword', 'success', function(act, json) {
                    me.keywordRender(json);
                    me.keywordBind();
                });

                $rk.listener.on('keyword', 'change', function() {
                    var id=$('.customMenu1Active').attr('data-id');
                    me.keywordRequest(id);
                });
            ...
            keywordRender:function(data){
                var html = $rk.tplRender(dot, $('#j_keyword_info').html(), data||{});
                this._container.find('.keywordConTipWrap').html(html);
            },
            keywordBind:function(){
                var me=this;
                //添加关键词
                $('.addKeyword').unbind().click(function(){
                    me.keywordAdd();
                });
                //删除关键词
                $('.delKeyword').unbind().click(function(){
                    var data={'keywordId':$(this).parent().attr('id'),'ruleId':$('.customMenu1Active').attr('data-id')};
                    me.keywordDel(data);
                });  
            },
            keywordAdd:function(){
                ...
                var dialog = new Dialog({
                    ...
                    content: '<div class="popBoxCon addCustomMenu1Con">'+
                                '<div>请设置关键词名称：</div>'+
                                '<div class="menuTxtWrap"><input class="text customMenu1Txt"></div>'+
                             '</div>',
                    confirmFun: function (dialog,button) {
                        var data={keyword:$('#j_dialog_'+dialog.guid).find('.customMenu1Txt').val(),id:$('.customMenu1Active').attr('data-id')};
                        me.keywordAddRequest(data);
            ...
            keywordDel:function(id){
                ...
                var dialog = new Dialog({
                    ...
                    content: '<div class="popBoxCon addCustomMenu2ConTip">'+
                                 ...
                                 '</div>',
                    confirmFun: function (dialog,button) {
                        me.keywordDelRequest(id);
                        dialog.close();
                    }
                });
                dialog.open();
            },
            keywordRequest:function(id){
                var option = {
                    url : keywordUrl,
                    dataType: 'json',
                    type: 'get',
                    data:{'id':id},
                    success: function(json) {
                       $rk.listener.trigger('keyword', 'success',json);
                    }
                }
                new Ajax(option); 
            },
            keywordAddRequest:function(data){
                var option = {
                    url : menu1AddUrl,
                    dataType: 'json',
                    type: 'post',
                    ...
                    success: function(json) {
                       var menu1AddRes=json.hfive_core_tag_customMenu_menu1Add_response;
                        if (menu1AddRes.code) {
                            $rk.listener.trigger('keyword', 'change');
                        }else{
            ...
            keywordDelRequest:function(data){
                var option = {
                    url : menuDelUrl,
                    dataType: 'json',
                    data: data,
            ...
            menuTreeBind:function(){
                ...
                //规则内容 
                $('.customMenuList li').click(function(){
                    var id,index=$(this).index('.customMenuList li');
                    if(index==0){
                        $('.keywordTitWrap').html('<span>关注回复语</span>');
                        $('.keywordConTipWrap').html('<span class="keywordConTip">粉丝关注后自动弹出的内容。</span>');
                        id=0;
                    }else if(index==1){
                        $('.keywordTitWrap').html('<span>默认回复语</span>');
                        $('.keywordConTipWrap').html('<span class="keywordConTip">粉丝回复了任意内容（除已设置规则的关键词）后，所回复的内容。</span>');
                        id=1;
                    }else{
                        $('.keywordTitWrap').html('<span>设置关键词</span><span class="addKeyword">+添加关键词</span>');
            ...
            menu1Add:function(){
                ...
                var dialog = new Dialog({
                    ...
                    content: '<div class="popBoxCon addCustomMenu1Con">'+
                             ...
                             '</div>',
                    confirmFun: function (dialog,button) {
                        var data={ruleName:$('#j_dialog_'+dialog.guid).find('.customMenu1Txt').val(),subscription:subscription};
            ...
            menu1Edit:function(id,selector){
                ...
                var customMenu1EditCon='<div class="popBoxCon addCustomMenu1Con">'+
                                                '<div>请设置规则名称：</div>'+
                                                '<div class="menuTxtWrap"><input class="text customMenu1Txt" value="'+$(selector).parent('.customMenu1Opr').next('.customMenu1').text()+'"></div>'+
                                             '</div>';
                var dialog = new Dialog({
                    title: '编辑回复规则',
                    content: customMenu1EditCon,
                    confirmFun: function (dialog,button) {
                        var data={ruleName:$('#j_dialog_'+dialog.guid).find('.customMenu1Txt').val(),id:id,subscription:subscription};
