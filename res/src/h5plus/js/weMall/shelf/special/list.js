...
*/
seajs.use(['Ajax','dot','tips2',"Dialog", 'clipboard','qrcode'], 
    function(Ajax,dot,Tips,Dialog,Clipboard,qrcode){
    ...
    var setModel = function() {
        ...
        var main = new $rk.VC({
            ...
            edit: function (that) {
                ...
                var store_id = that.parent().find('input').attr('data-store_id');
                var page_id = that.parent().find('input').attr('data-page_id');
                window.location.href = details + '?id=' + id + '&store_id=' + store_id + '&page_id=' + page_id;
            },
            // 下架
            ...
            delete: function (that) {
                ...
                var dialog = new Dialog({
                    title: '删除页面',
                    content: '<p class="ui-msg ui-msg-warning">你确定要删除页面“xxx”吗?删除页面将会删除页面所有内容。</p>',
                    confirmFun: function (dialog,button) {
            ...
            formSend :function(){
                ...
                var option = {
                    ...
                    type: 'post',
                    success: function(json) {
                        if(json.hfive_weidian_page_list_response){
