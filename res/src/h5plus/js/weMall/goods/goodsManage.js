    ...
    var setModel = function() {
        ...
        var form = new $rk.VC({
               ...
               var option = {
                    ...
                    success: function(json) {
                        if(!json.error_response){
                            ...
                            json.error_response.code=0;
                            $rk.listener.trigger('tipsPhoton', 'success',json.error_response);
                            $rk.listener.trigger('brandList', 'success', {});
                        } 
                    }
                }
                new Ajax(option); 
            ...
            imgFileRequest:function(param){
                ...
                XMLHttpReq.withCredentials = true;  //支持跨域发送cookies
        ...
        var main = new $rk.VC({
            ...
            showBrandModal2: function(dom){
                dom.attr('data-target','#brandAddEdit');
                var brand_id = dom.parent().find('input[type=hidden]').val() - 0 ;
                var data = {"brand_id": brand_id};
                var option = {
                     url : getBrandInfoUrl,
                     type: 'post',
            ...
            bindGoodsAddEditTwo: function(){
                ...
                var v = $('#j_flags_ae').val();
                if(v == 1){ // edit
                    var cate_id =detailData.cat_id; 
                    var product_id =detailData.product_id; 
                    var option = {
                        url: attrListUrl,
                        type: 'post',
                        dataType:'json',
            ...
            goodsAddEditTwo_attrListOpt: function(){
               var cate_id =finalData.category_id - 0 ;
            ...
            addSigle: function(dom){
                ...
                var pid = dom.parents('.j_goodAttr_sigle').find('select').val() - 0 ;
            ...
            readerTags: function(data){
                ...
                var container_id = _dom.parent('div').prev('div.sigleBox').parents('div.j_goodAttr_sigle');
                container.append(html);

                container_id.find('div.sigle').each(function(index, el) {
                    /*var dom = $(el).attr('id');
                    console.log(dom);*/
                    me.imgUpLoad($(el)); 
                });
            ...
            collectSigleData: function(){
                ...
                $('.goodAttrCon').find('div.j_goodAttr_sigle').each(function(index, el) {
                   properties_Arr.push({});
                   var sele_v = $(el).find('select option:selected').val() - 0;
            ...
            finishAdd: function(dom){
                ...
                var obj_1_nv = {},obj_2_nv = {},obj_3_nv = {},obj_4_nv = {};

                // 拼数据    规格的数据
                if(v == 0){
                    var len =  finalData.properties.length;
                    } // add
                else{
                    var len = detailData.properties.length;
                    }  // edit 
                if(!finalData.properties){me.collectSigleData();}
                ...
                if(len==1){
                    ...
                    for (var i = ar.length - 1; i >= 0; i--) {
                        ...
                        obj_1_nv.price = parseFloat(price);
                    ...
                    for (var i = ar.length - 1; i >= 0; i--) {
                        ...
                        $('#j_goodsSigleList tbody tr[id=j_'+i+']').each(function(index, el) {
                            ...
                            obj_2_nv.price = parseFloat(price2);
                    ...
                    for (var i = ar.length - 1; i >= 0; i--) {
                        ...
                        for(var d = ar2.length - 1; d >= 0 ; d--){
                                ...
                                obj_3_nv_2.pid = finalData.properties[2].pid -0;
                            ...
                            for(var h = ar3.length - 1; h >= 0 ; h--){
                                    ...
                                    obj_4_nv_2.price = parseFloat($('#j_goodsSigleList tbody').find('tr').eq(h).find('td').eq(n-2).find('input').val());
                ...
                if(v == 0){
                    ...
                    saveUrl = goodsEditUrl;
                    finalData.product_id = detailData.product_id - 0;
                    finalData.property_is_changed = true;
                    }  // edit
                ...
                var option = {
                    ...
                    success: function(json) {
                        if(!json.error_response){
                            var brandId  = $('input[data-id=brandId]').val() - 0;
                            me.awardListRequest({ "page": 1,"page_size": 10,"brand_id": brandId});
                            $('#goodsAddEditLast a[data-dismiss=modal]').click();
                        }
                    }
                }
                new Ajax(option);
            ...
            removeGoodRequest: function(button,that){
              var me = this;
              var product_id = that.parent().find('input[type=hidden]').val() - 0;
              var brandId  = $('input[data-id=brandId]').val() - 0;
              var btn = button;
              var self = this;
              var tip = null;
