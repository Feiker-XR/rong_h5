seajs.use(['Ajax', 'dot', 'migrate', 'pagination', 'tips2', "Dialog", 'sortable', 'upload', 'zTree', 'zclip', 'zThide', "jstree", "dataTables", "easyui", "jqui", "bootstrap", "bootstrap_fileupload","swiper_min","query_qrcode_min","qrcode","hammer_min","mobilelable","commoditylable","jquery_Jcrop","jquery_color","imgedit","jquery_pnotify_min","mCustomScrollbar"],
	function(Ajax, dot, migrate, pagination, Tips, Dialog, sortable, upload, zTree, zclip, zThide, jstree, dataTables, easyui, jqui, bootstrap, bootstrap_fileupload,swiper_min,query_qrcode_min,qrcode,hammer_min
		,mobilelable,commoditylable,jquery_Jcrop,jquery_color,imgedit,jquery_pnotify_min,mCustomScrollbar) {
	var page_size, //共page_size页
			...
			opFlag,//创建标签还是添加图片
			opFlagTag,//
			opSwitchcuiimg,//如果是创建标签则保持当前图片的id  应为在swiper后，再创建标签  会刷新图片列表这样子cuiiimg就乱了
...
//									notifications(obj, "suc");
									//opFlagcuiimg="creatTag";
//									if(opSwitchcuiimg=="switchImg")
//										opSwitchcuiimg="creatTag";
									//创建标签成功:0516注释掉   创建标签之后暂定不需要刷新图片
									 ...
									 if(opFlagTag=="edit"){
										$("#boxid"+tag_id_edit).remove();//删除标签的时候  删除掉对应的图片上的标签
										createtopic.delwood({//再调用后台接口 做到数据同步
											"tag_id": tag_id_edit
										});
										opFlagTag="end";
										 var obj = {
...
//						var htmlstr = '<div style="float: left;margin: 10px;"><img src="' + (obj.album_pic_url?obj.album_pic_url:"") + '" style="width: 100px;height: 100px;"></div>' + '<div style="float: left;" class="album_head">  ' + '<div><span>专辑名称:</span><span>' + obj.album_name + '</span></div> ' + ' <div><span>专辑描述:</span><span>' + (obj.album_desc?obj.album_desc:'----') + '</span></div> ' + '  <div><span>话题数:</span><span>' + obj.album_topic_count + '</span></div> ' + '</div>';
						var html_img= '<div style="float: left;margin: 10px;"><img src="' + (obj.album_pic_url?obj.album_pic_url:hostUrl+"/static/h5plus/img/wechatBBS/mobilefront/album.png") + '" style="width: 100px;height: 100px;"></div>';
						var htmlstr = '<div style="float: left;" class="album_head">  ' 
					...
					_bind: function() {
						...
						this._container.on("click", '.bntos', function(e) {
							...
							if ($(e.target).attr('class').indexOf("delstyle") > 0) {
								...
								$("#select-img-album").off("click").on("click",function(){
									$("#inputimgpath").click();
								});
								$rk.listener.trigger('albumdlg', 'getdata');
								$("#inputimgpath").val("");
					...
					render: function(obj) { //加载话题表格22222222
						...
						if (obj.topics != null) {
							for (var i = 0; i < obj.topics.topic.length; i++) {
								htmlstr += '<tr>' 
										+ '<td><input name="checkbox" type="checkbox" style="margin-top: -4px;" value="'+ obj.topics.topic[i].topic_id+ '">' 
...
//						new Ajax(option);
				...
				var createtopic = new $rk.VC({
					...
					_renderimg: function(obj) {
						...
						var activeImgNo=0;
						//如果是创建标签则保持当前图片的id  应为在swiper后，再创建标签  会刷新图片列表这样子cuiiimg就乱了
						for (var i = 0; i < obj.imgs.img.length; i++) {
								...
								if(obj.imgs.img[i].img_id==cuiimgid){
									activeImgNo=i;
								}
						...
						var imgloading='<div id="forLoading" style="position:absolute;z-index:9999;'
										+'width:291px;height:200px;line-height:200px;text-align:center;color:red;margin-top: -200px;background:rgba(0,0,0,0.8)">'
										+'<img src="'+hostUrl+'/static/h5plus/img/wechatBBS/list/325.gif" style="text-align:center;width:40px;height:40px;">'
										+'</div>';
						$(".topicimgarr").append(imgloading);
						
//						if(obj.imgs.img.length>0){
//							cuiimgid=obj.imgs.img[0].img_id;
//						}
						console.log("当前 应该显示的图片所属层级：："+activeImgNo);
					 	setTimeout(function(){
					 		if(obj.imgs.img.length>0){
					 			$("#forLoading").remove();
					 		}
							var swiper = new Swiper('.swiper-container', {
								...
								noSwiping: true,//拖拽滑动 可能会与标签拖拽有冲突  所以这里取消拖拽
								loop : false,
								initialSlide:activeImgNo,
								onSlideChangeEnd: function(swiper) {
									var _cuiimgid=$(".swiper-slide-active").attr("data-id");
									//初次加载显示第一张图片但是在此切换的时候 开始加载其他的图片
									 cuiimgid=$(".swiper-slide-active").attr("data-id");
										_this.gettags({
											"topic_id": curtopicid,
											"topic_img_id":cuiimgid
										},function(obj) {
											_this._rendrtag(obj, _cuiimgid);
										});
						...
						var option = {
							...
							success: function(json) {
								if (json["hfive_community_topicimg_findlist_response"] != undefined) {
									var json_len=json["hfive_community_topicimg_findlist_response"].imgs.img.length;
									if(json_len>0){//再删除图片时候  若不加在个判断，则在删除所有的图片时会报错，而且最后一个图片删除不掉
										cuiimgid=json["hfive_community_topicimg_findlist_response"].imgs.img[json_len-1].img_id;
									}else{
										cuiimgid=0;
									}
