...
(function(root, factory) {
	...
	$.Dialog = function(settings) {
		...
		if (settings.type === "confirm") {
			...
			action = '<div class="ui-dialog-action" style="width:100%;"><div>' + btnstr + '</div></div>';
			html += action;
			var options = $.extend({
				target: html,
				animate: true,
	...
	Dialog.prototype = {
		...
		hide: function() {
			...
			if (this.settings.animate) {
				//this.dialogContainer.removeClass('zoomIn').addClass("zoomOut");   // 弹出效果
				setTimeout(function() {
		...
		show: function() {
			...
			if (this.settings.animate) {
				//this.dialogContainer.addClass('zoomIn').removeClass('zoomOut').addClass('animated');  // 弹出效果
			}
		},
