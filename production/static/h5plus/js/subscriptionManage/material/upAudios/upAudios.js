...
function(Ajax,dot,pnotify,bootstrap,zclip){
    ...
    var setModel = function() {
        ...
        var soundListUrl = $$r.routers.data.link.soundList,
            soundDeleteUrl = $$r.routers.data.link.soundDelete,
            soundAddUrl = $$r.routers.data.link.soundAdd;
        ...
        var form = new $rk.VC({
            ...
            bind:function(){
                ...
                $('.panel-slider-arrow').off().on('click', function(){
                    ...
                    $('.audioContainer').toggleClass('contentRetracted');
                });
            ...
            bindSoundList:function(){
                var me=this;
                //分页
                page_size=$('#soundTablemain').attr('page_size');
                ...
                //播放
                $('a[class=playSound]').unbind('click').click(function(){
                    var sound_url=$(this).attr('sound_url');
                    $('.audioTitle').html($(this).attr('title'))
                    var str= '<audio controls="controls" autoplay="autoplay" id="myAudio"><source src="'+sound_url+'">你的浏览器不支持video标签。</audio>';
                    $('#audioSoundtab').html(str);
                    //console.log(sound_url)
                });
                //复制链接
                /*$('a[class=copySoundpath]').click(function(){
                    console.log($(this).attr('path'))
                })*/
                $('a[class=copySoundpath]').zclip({
                ...
                //删除
                $('a[class=deleteSound]').unbind('click').click(function(){
                    $('#j_p').nextAll('.ui-pnotify ').hide();
                    $('#j_p').nextAll('.ui-pnotify ').remove();
                    ...
                    $('#upVideobtn').unbind('click').click(function(){
                        ...
                        function lastname(){
                            ...
                            if(rs>=0){
                                ...
                                $('#fileName').val('')
                                $rk.listener.trigger('tips', 'success','上传的文件仅支持amr、mp3两种格式！');
                                $('.ui-pnotify ').css({'top':'40px'})
                                return false;
                ...
                $('#canselSound').unbind('click').click(function(){
                    ...
                    if(XMLHttpReq.readyState==4){
                        if(XMLHttpReq.status==200){
                            if(JSON.parse(XMLHttpReq.responseText).callResult){
                                ...
                                $('#upVideobtn').attr('disabled',false);
                                var fileData={title:JSON.parse(XMLHttpReq.responseText).file_name,sound_url:JSON.parse(XMLHttpReq.responseText).oss_path,size:JSON.parse(XMLHttpReq.responseText).file_size}
                                    me.soundAdd(fileData)
                            }else{
                                $('#fileName,#files').val('');
