...
RKC.seaConfig = {
    base: $$r.rootUrl,
    alias: {
        // "zepto": $$r.rootUrl + '/comlibjs/lib/zepto.js',
        ...
        'sortable': $$r.rootUrl + '/comlibjs/module/sortable/1.0.1/sortable.js',
        ...
        // 'h5sdk': 'https://test.h5plus.net/frontend/js/h5sdk1.00.js',
        'h5sdk': $$r.rootUrl + '/comlibjs/mobile/h5PHPApiSdk.js',
        ...
        'iscroll': $$r.rootUrl + '/comlibjs/mobile/iscroll.js',
        '$swiper': $$r.rootUrl + '/comlibjs/mobile/$idangerous.swiper.min.js',
        '$swiper3d': $$r.rootUrl + '/comlibjs/mobile/$idangerous.swiper.3dflow.js',
        '$dialog': $$r.rootUrl + '/comlibjs/mobile/$dialog.js',
        'areaData': $$r.rootUrl + '/comlibjs/mobile/area.min.js',
        '$selectArea': $$r.rootUrl + '/comlibjs/mobile/$selectArea.js'
    }
}
 ...
 */
RKC.mobile = {
    isWeChat : /MicroMessenger/g.test(window.navigator.userAgent),
    resize : function () {
        var fontBase = arguments[0] || 14,
        clientBase = arguments[1] || 320,
        docEl = document.documentElement,
        resizeEvt = 'orientationchange' in window ? 'orientationchange' : 'resize',
        recalc = function () {
          var clientWidth = docEl.clientWidth;
          if (!clientWidth) return;
          docEl.style.fontSize = fontBase * (clientWidth / clientBase) + 'px';
        };
        if (!document.addEventListener) return;
        window.addEventListener(resizeEvt, recalc, false);
        document.addEventListener('DOMContentLoaded', recalc, false);
    },
    mainSuffix : window.location.port === '1221'?'':'.html'
    ,pid_test : '55cbf3a3986a9b483376f279'	//测试环境,
    ,pid_online : '55cc2ada0a4f42e010250333'	//正式环境
}
...
var $Rk = window.$Rk = $rk = {
    ...
    setUrlParam: function(name, value) {
        ...
        var searchSubAll = window.location.search.substr(1).replace(name+'='+r[2], name+'='+value)
        var formatedUrl = window.location.origin +location.pathname +'?'+ searchSubAll
        if (r != null) return searchSubAll;
        return null;
    },
    /**
     * 取url参数
    ...
    /**
     * [LSO 本地存储(Local Shared Object)]
     * @param  {String} key [变量字段]
     * @param  {String} val [值]
     * @return  {String} val [值]
     */
    LSO: function (key , val) {
    	var isVal = val==null || val==false || val=='' || val==0,
    		isObject = function (v) {
    	...
    	if(arguments.length===1){
    		...
    		return isObject(window.localStorage[key])?window.localStorage[key]:isJSONString(window.localStorage[key])
    	}else if(key && !isVal){
    ...
    plat: {
        ...
        loading: function() {
            ...
            // console.log('hash_get:',$rk.hash.get())
            // $rk.plat.ready();
            if ($rk.hash.get()) {
                // alert($rk.hash.get())
                // $rk.plat.ready();
        ...
        s : function(callback,src){
            src = src || $('#j_js').get(0) && $('#j_js').attr('src').split('?v')[0].replace(/\.js$/,'.html');
        ...
        /**
         * [Object数据转url参数  eg: $.fn.serialize, $.fn.serializeArray]
         * @param  {[type]} form [description]
         * @return {[type]}      [description]
         */
        serializeURI : function(obj) {
			var params = [];
...
//设置每个VC
$Rk.VC = function(op) {
    $.fn.extend&&$.fn.extend(this, op)||$.extend(this, op);
    $Rk.BassPlat.call(this);
}
