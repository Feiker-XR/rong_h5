...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			var array=[];
			//绑定文件加入队列事件;
			webUploader.on('fileQueued', function( file,response ) {
				var file_id = file.id
				$('#upPicbut').attr('disabled',false)
				createBox( $fileInput, file ,webUploader);
				var file_indexParam={};
                    file_indexParam.inputName="file";
                    file_indexParam._this=file.source.source;
                    file_indexParam.method="POST";
                    file_indexParam.url='http://'+window.location.host+'/auth-portal/api/upload?method=ruixue.hfive.core.filemanager.activity.picture.upload';
                    videoUpload(file_indexParam,file_id);
			});
			//进度条事件
			...
			webUploader.on('error', function( code ) {
				...
				switch( code ) {
					...
					break;
					case 'Q_TYPE_DENIED' : text = '文件类型不正确!';
					break;
					default : text = '未知错误!';
 					break;	
				}
				$rk.listener.trigger('tips', 'success',text); 
				$('.ui-pnotify ').css({'top':'40px'})
            	//alert( text );
        	});
        	function videoUpload(param,file_id){
                var XMLHttpReq=null;
                    ...
                    if(XMLHttpReq.readyState==4){
                        if(XMLHttpReq.status==200){
                            if(JSON.parse(XMLHttpReq.responseText).callResult){
                               ...
                               var li = '<li id="fileBox_'+file_id+'" file_size="'+JSON.parse(XMLHttpReq.responseText).file_size+'" class="diyUploadHover" data-name="'+JSON.parse(XMLHttpReq.responseText).file_name+'"> \
											<div class="viewThumb"><img src="'+JSON.parse(XMLHttpReq.responseText).oss_path+'"></div>\
											<div class="diyCancel"></div> \
											<div class="diySuccess"></div> \
											<div class="diyFileName">'+JSON.parse(XMLHttpReq.responseText).file_name+'</div>\
											<div class="diyBar"> \
													<div class="diyProgress"></div> \
													<div class="diyProgressText">0%</div> \
											</div> \
										</li>';
								$('.fileBoxUl').find('#upfile').after( li )
								$('.fileBoxUl div.diyCancel').click(function(){
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		//添加子容器;
		var file_Name = (file.name).replace(/\.(jpg|gif|png)$/i,"")
		var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover" data-name="'+file_Name+'"> \
		...
		//$parentFileBox.children('.fileBoxUl').find('#upfile').after( li );
		...
		webUploader.makeThumb( file, function( error, dataSrc ) {
			if ( !error ) {	
				...
				/*var b64s=JSON.stringify({'imgData':dataSrc});
					$.ajax({
					type:"POST",
					url:'http://h5plus.net/auth-template/api/upload?method=ruixue.hfive.core.filemanager.activity.picture.upload.json&pid='+'55cc2ada0a4f42e010250333',
					//url:$$r.online + '/auth-portal/api/upload?method=ruixue.hfive.core.filemanager.activity.picture.upload.json',//&pid='+'55cbf3a3986a9b483376f279',
					...
					success: function(data){
						$fileBox.find('.viewThumb').append('<img file_name="'+data.file_name+'" file_size="'+data.file_size+'" src="'+data.oss_path+'" base64="'+dataSrc+'">');							
					} })*/
