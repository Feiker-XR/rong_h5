define(RKC.seaConfig.alias.upload, ['../../json2/json2'], function(require, exports, module) {
    ...
    var Upload = function(opt) {
        opt = $.extend({
            ...
            max: 5, //多文件 最多选几个
            url: opt.url || $rk.url.upload, //配置上传地址 by chenyiqi
            fileQueued: function() {
    ...
    $.extend(Upload.prototype, {
        ...
        getFilesLen: function() {
            return $Rk.getJsonLen(this.files);
        },
        send: function() {
        ...
        _sendFilesItem: function(key, file) {
            ...
            this.xhr[key].upload.onprogress = function(e) {
                //每次添加ui-button-disabled类会有闪动
                //me.label.addClass('ui-button-disabled');
                me.label.text('上传中...');
            ...
            this.xhr[key].onreadystatechange = function() {
                ...
                if (me.xhr[key].readyState == 4 && me.xhr[key].status == 200) {
                    var json = json2.parse(me.xhr[key].responseText);
